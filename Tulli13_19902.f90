...
implicit none
  ...
  real*8,dimension(:), allocatable :: pos,v,old_acc2,mass
...
do p=1,Hi
acc1(p,acc1i)=(-sum(Energy_hamil(:,p)*matmul(Gradient_hamil(:,:,acc1i),Energy_hamil(:,p))))/mass(acc1i)
end do
end do
end subroutine
!..........................................................................
...
implicit none
integer :: r,s,t,reciprocal_mass_loop,v_loop
integer, intent(inout) :: ip,res,r1,ser,r0,per,l1,rep,l0,ks
real*8 :: para_v          !para_v= component of v parallel to acw
real*8, dimension(:),allocatable :: perp_v,reciprocal_mass 
real*8, dimension(Hi,Hi) :: hp
real*8 :: a,b,frustrated_condition,gama
allocate(perp_v(total_dimensions))
allocate(reciprocal_mass(total_dimensions))
ip=ks                     
v=v*ip
...
do t=1,Hi
   if ((t.ne.lambda).and.hp(lambda,t)>rnd) then
       a=0.5*sum(reciprocal_mass*acw(lambda,t,:))
       b=sum(v*acw(lambda,t,:))
       frustrated_condition=b**2+4*a*(Energy(lambda)-Energy(t))
       if (frustrated_condition>0) then
           ...
           do v_loop=1,total_dimensions
             v(v_loop)=v(v_loop)-gama*acw(lambda,t,v_loop)/mass(v_loop)
           end do
           lambda=t
        end if
    end if
...
!do t=1,Hi
!if (t.ne.lambda) then
!if (hp(lambda,t)>rnd.and.((sum(v*acw(lambda,t,:))/norm2(acw(lambda,t,:)))**2+(2*Energy(lambda)/mass(1))-(2*Energy(t)/mass(1)))>0) then
!    para_v=sum(v*acw(lambda,t,:))/norm2(acw(lambda,t,:))
!    perp_v=v-para_v*acw(lambda,t,:)/norm2(acw(lambda,t,:))
!    para_v=((para_v)/abs(para_v))*sqrt(para_v**2+(2*Energy(lambda)/mass(1))-(2*Energy(t)/mass(1)))
